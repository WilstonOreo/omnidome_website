...
/**
 * Contact form AJAX sumbission
 */
$(document).ready(function() {
  var contactFormSuccess = function(response) {
    $("#contact-form-success").slideDown();
  }

  var contactFormError = function(response) {
  ...
  $('.page-contact button.close').click(function(event) {
    $(event.target).parent().slideUp();
  });

  $('#contact-form').attr('action', '/entry_submissions/customer_messages.json');
...
$(document).ready(function(){
  var addTweets = function(tweets) {
    tweetList = $.map(tweets, function(item) {
      return "<li>" + item + "</li>"
    });

    $("#twitterfeed ul").append(tweetList);
    $("#twitterfeed .interact a").addClass('btn btn-primary btn-mini');

    $('#twitterfeed ul').bxSlider({
      mode: 'vertical',
      auto: true,
      minSlides: 1,
      responsive: true,
  ...
  twitterFetcher.fetch('447479917442105344',
                       'twitterfeed',
                       5,
                       true,
                       false,
